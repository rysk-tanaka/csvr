...
2. **レイアウト計算・データ変換** — `compute_column_widths`（先頭100行サンプリングで列幅算出）、`row_number_col_width`（行番号列の桁幅算出）、`filter_rows`（クエリによる行フィルタリング）、`compute_numeric_columns`（全行ベースで列ごとの数値判定）、`sort_indices`（列ソート）、`extract_column_values`（列から数値値抽出）、`extract_scatter_pairs`（2列同時抽出）、`downsample`（均等間引き）、`compute_histogram_bins`（ヒストグラムビン計算）。テスト可能な純粋関数
