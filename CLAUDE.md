...
2. **レイアウト計算** — `compute_column_widths`（先頭100行サンプリングで列幅算出）、`row_number_col_width`（行番号列の桁幅算出）、`filter_rows`（クエリによる行フィルタリング）、`sort_indices`（列ソート、数値列は数値比較）。テスト可能な純粋関数
...
- `toggle_sort(col)` — ソート状態サイクル（None → Asc → Desc → None）+ `recompute_filtered_indices`
